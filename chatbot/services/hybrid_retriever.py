...
class HybridRetriever(BaseRetriever):
    ...
    def __init__(
        ...
        if not os.path.exists(abs_index_dir):
            print(f"Whoosh index directory not found at {abs_index_dir}")
            raise FileNotFoundError(
                f"Whoosh index not found at {abs_index_dir}. "
                "Please run one of the following commands to create the index:\n"
        ...
        try:
            ...
            print(f"Error opening Whoosh index: {str(e)}")
            raise Exception(
                f"Whoosh 인덱스를 여는 중 오류가 발생했습니다: {str(e)}\n"
                f"인덱스가 손상되었을 수 있습니다. 다음 명령어로 재생성해주세요:\n"
                "python manage.py run_whoosh_index"
            )
